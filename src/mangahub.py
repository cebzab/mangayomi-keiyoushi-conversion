...
url = f"{BASE}/search?keyword={q}"
soup = _get_soup(url)
results = []
for it in soup.select('.item'):
a = it.select_one('.thumb a')
if not a:
continue
title = it.select_one('.item-title').get_text(strip=True) if it.select_one('.item-title') else a.get('title', '')
...
soup = _get_soup(BASE + '/popular')
results = []
for it in soup.select('.item'):
a = it.select_one('.thumb a')
link = urljoin(BASE, a['href'])
title = it.select_one('.item-title').get_text(strip=True) if it.select_one('.item-title') else ''
thumb = it.select_one('img').get('src') if it.select_one('img') else None
results.append({'id': link, 'title': title, 'url': link, 'thumbnail': thumb})
return results
...
thumb = soup.select_one('.thumb img').get('src') if soup.select_one('.thumb img') else None
info = {
'title': title,
'author': '',
'artist': '',
...
soup = _get_soup(manga_url)
chapters = []
for a in soup.select('.list-chapters a'):
